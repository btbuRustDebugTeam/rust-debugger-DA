...
// --- Log stream ---
{
    const r = parseMILine('&"warning: something\\n"');
...
// --- Target stream ---
{
    const r = parseMILine('@"target output"');
...
{
    const r = parseMILine('^error,msg="No symbol table"');
...
// --- Exec async: *stopped ---
{
    const r = parseMILine('*stopped,reason="breakpoint-hit",bkptno="1",frame={func="main",line="5"}');
...
// --- Exec async: *running ---
{
    const r = parseMILine('*running,thread-id="all"');
...
{
    const r = parseMILine('=thread-created,id="1",group-id="i1"');
...
// --- Result with list ---
{
    const r = parseMILine('5^done,threads=[{id="1",name="main"},{id="2",name="worker"}]');
...
{
    const r = parseMILine('^done,frame={level="0",func="main",file="main.rs",fullname="/home/user/main.rs",line="10"}');
...
{
    const r = parseMILine('^error,msg="No symbol \\"foo\\" in scope"');
...
// --- Stack frames list ---
{
    const r = parseMILine('10^done,stack=[frame={level="0",func="leaf"},frame={level="1",func="nonleaf"}]');
...
// --- Unrecognised line ---
{
    const r = parseMILine('some random text');
