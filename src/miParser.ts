...
export interface MIRecord {
    ...
    /** Numeric token that was prefixed to the command (undefined if none). */
    token?: number;

    /**
     * The class/reason word that follows the record prefix character.
     * e.g. "done", "error", "stopped", "running", "thread-created", ...
     * Undefined for stream records and prompt.
     */
    cls?: string;

    /**
     * Parsed key-value data that follows the class word.
     * For stream records this is `{ msg: "<content>" }`.
     */
...
export function parseMILine(line: string): MIRecord | undefined {
    ...
    if (trimmed[0] === '~' || trimmed[0] === '@' || trimmed[0] === '&') {
        const typeMap: Record<string, MIRecordType> = {
            '~': 'console-stream',
            '@': 'target-stream',
            '&': 'log-stream',
        };
 ...
 */
function parseMITuple(input: string): Record<string, any> {
 ...
 */
function parseMIValue(input: string, pos: number): [any, number] {
    if (pos >= input.length) {
        return ['', pos];
    }
 ...
 */
function parseMICString(input: string, pos: number): [string, number] {
    if (input[pos] !== '"') {
        return ['', pos];
    }
 ...
 */
function parseMITupleValue(input: string, pos: number): [Record<string, any>, number] {
    if (input[pos] !== '{') {
        return [{}, pos];
    }
 ...
 */
function parseMIListValue(input: string, pos: number): [any[], number] {
    if (input[pos] !== '[') {
        return [[], pos];
    }
 ...
 */
function findNextUnquoted(input: string, start: number, target: string): number {
    let inString = false;
    let escaped = false;
