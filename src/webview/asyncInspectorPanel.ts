...
export class AsyncInspectorPanel {
    ...
    private _activeTreeView: 'history' | 'observer' = 'observer';
    ...
    private async handleContinueExecution(): Promise<void> {
        const session = this._debugAdapterFactory?.getActiveSession();
        if (!session) {
    ...
    private async fetchObserverTree(suppressOutput: boolean = false): Promise<ObserverTreeData | undefined> {
        const session = this._debugAdapterFactory?.getActiveSession();
        if (!session) {
    ...
    private renderRuntimeObserverTree(observerTree: ObserverTreeData): void {
    ...
    private postActiveTree(observerRoot?: string | null): void {
    ...
    private async handleRefreshObserver(): Promise<void> {
