...
| **初赛PPT** | [操作系统跨特权级统一调试平台初赛PPT](https://gitlab.eduxiji.net/T2026100119910438/project3136859-387115/-/blob/main/%E5%88%9D%E8%B5%9BPPT-%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F%E8%B7%A8%E7%89%B9%E6%9D%83%E7%BA%A7%E7%BB%9F%E4%B8%80%E8%B0%83%E8%AF%95%E5%B9%B3%E5%8F%B0.pdf)                                                       |
...
（3）将两项能力合并，在异步操作系统中验证跨特权级切换管理与异步执行流还原的协同工作；

（4）把调试对象从 QEMU 扩展到真实 RISC-V 开发板，建立 VisionFive 2 真机调试链路，并将真实设备纳入线上硬件交互平台的预约和调度体系。
...
| [async-debug] 异步操作系统统一调试（rel4） | **全部完成** | 在 Async-os 场景下同时启用断点组自动切换与异步跟踪，验证跨特权级协程追踪连续性及逻辑调用树展示 |
| [硬件调试] VisionFive 2 真机调试链路 | **全部完成** | 建立 J-Link、OpenOCD、GDB 与 USB-TTL 串口组成的真机调试链路，支持处理器状态读取、暂停、恢复及跨特权级单步调试 |
| [线上平台] 真实开发板预约与调度 | **全部完成** | 实现 VisionFive 2 的预约、分配、释放、SSH 公钥注册、状态更新、日志与会话归档；硬件调试服务自动编排仍在扩展 |
...
| 王浩铭   | OS 调试功能测试与验证（状态机单元测试、集成测试）；VisionFive 2 硬件调试环境搭建与适配；线上硬件交互实验平台设计与实现；真实开发板接入、预约调度与会话管理；硬件调试章节撰写 |
| 武雪妍   | 图片制作、PPT 制作 |
...
- [操作系统跨特权级统一调试平台](#操作系统跨特权级统一调试平台)
  ...
  - [三、核心目标与完成情况](#三核心目标与完成情况)
    ...
    - [3.4 异步操作系统统一调试](#34-异步操作系统统一调试)
    - [3.5 VisionFive 2 真机调试与线上化扩展](#35-visionfive-2-真机调试与线上化扩展)
    - [3.6 后续工作与展望](#36-后续工作与展望)
  - [四、核心工作分述](#四核心工作分述)
    ...
    - [4.5 VisionFive 2 真机调试与线上硬件交互平台](#45-visionfive-2-真机调试与线上硬件交互平台)
  - [五、测试与验证](#五测试与验证)
    - [5.1 测试环境](#51-测试环境)
    ...
    - [5.6 Async-os 统一调试场景验证](#56-async-os-统一调试场景验证)
  - [六、总结与展望](#六总结与展望)
    - [6.1 核心成果](#61-核心成果)
    - [6.2 未来展望](#62-未来展望)
  - [七、功能展示](#七功能展示)
...
4. **真实硬件调试与线上化管理**。当调试对象由 QEMU 迁移到真实 RISC-V 开发板时，需要通过 JTAG 控制处理器，并利用串口观察系统输出；同时还要解决真实设备的预约、分配、释放和会话归档问题，使有限的硬件资源能够支持远程教学实验。
...
- [X] **目标三：将两项能力合并，完成异步操作系统统一调试验证**。同时启用特权级切换管理和异步执行流还原，在 Async-os 上验证断点组自动切换、跨特权级协程追踪连续性和逻辑调用树展示。
- [X] **目标四：把调试能力扩展到真实硬件与线上实验环境**。建立 VisionFive 2、J-Link、OpenOCD、GDB 与串口组成的真机调试链路，并把真实开发板纳入线上平台的预约和调度体系。

![项目总体方案：核心改进、两个调试子系统与验证场景共享 GDB/MI2 协议层](docs/assets/项目总览图.png)
...
异步操作系统调试需要同时启用特权级切换管理（四状态机 + 断点组）和异步执行流还原（await edge + call edge 恢复）。目前已在 rel4 异步操作系统场景中完成协同验证：内核态与用户态之间的断点组和符号表能够自动切换，协程 ID、poll 次数和追踪记录在特权级切换前后保持连续，Async Inspector 能够展示跨内核态与用户态的逻辑调用树。

### 3.5 VisionFive 2 真机调试与线上化扩展

项目在已有工作的基础上，对 VisionFive 2 的硬件调试功能进行了复现和适配。真机链路由 GDB、OpenOCD、J-Link 和 VisionFive 2 组成：OpenOCD 在本机 3333 端口提供 GDB Server，J-Link 通过 JTAG 向处理器发送断点、单步、寄存器读取、暂停和恢复命令；USB-TTL 串口独立承担 U-Boot 与目标操作系统输出观察。

调试环境采用 J-Link V12 与 OpenOCD 0.12.0，能够识别 VisionFive 2 的 E24、U74 调试模块和 5 个 XLEN=64 的 hart。针对真实硬件单步期间 `dcsr.stepie=0` 且相关 CSR 无法通过 OpenOCD 改写的问题，采用边界断点与断点组切换机制，在 `ecall` 前保存用户态调试状态，在进入内核地址空间后启用内核断点组，并通过内核出口边界恢复用户态断点，实现内核态、用户态单步及跨特权级双向切换。

在此基础上，项目设计并实现了面向 VisionFive 2 的线上硬件交互实验平台。平台以 Moodle 作为教学入口，Flask 提供注册、预约和查询服务，MariaDB 保存用户、预约、设备及调度状态，Docker 提供隔离的实验环境；同时集成 SSH、TFTP 和 RISC-V 工具链。平台已经能够完成真实开发板的预约、分配、释放、提前提醒、状态更新、终端日志和会话归档。
...
- [ ] **硬件调试服务自动编排**。把 OpenOCD 进程启停、GDB 连接信息和串口日志进一步纳入线上平台调度流程，使用户获得开发板权限后能够直接开展远程操作系统调试。
...
### 4.5 VisionFive 2 真机调试与线上硬件交互平台

真实硬件调试采用“GDB - OpenOCD - J-Link - VisionFive 2”控制链路和独立 USB-TTL 串口链路。GDB 加载目标系统符号并发送调试命令，OpenOCD 将远程协议转换为 JTAG 操作，J-Link 完成处理器控制，串口用于观察 U-Boot 和目标操作系统输出。JTAG 扫描能够识别 5 个 RISC-V hart；GDB 可以查看线程状态和 PC、SP、RA 等寄存器，并控制处理器暂停与恢复。

线上硬件交互平台负责真实设备的使用权管理。用户注册 SSH 公钥并预约实验时段后，调度服务在预约开始时分配 VisionFive 2，在结束时停止会话、恢复设备状态、归档终端日志并释放开发板。平台查询页面可显示预约时段、SSH 连接信息和 RISC-V 架构。后续将进一步把 OpenOCD、GDB 和串口日志自动纳入预约会话，实现完整的远程操作系统调试流程。
...
### 5.6 Async-os 统一调试场景验证

在 rel4 异步操作系统上同时启用 OS 调试与异步跟踪后，调试器完成了以下验证：

- **断点组自动切换**：内核初始化、进入用户态、用户程序发起系统调用和返回内核态的完整链路中，断点组与符号表均自动切换。
- **跨特权级追踪连续性**：保存与恢复机制保证协程 ID 不丢失、poll 次数累计不间断，await edge 能够跨地址空间关联异步操作。
- **逻辑调用树跨特权级展示**：Async Inspector 能够展示跨内核态和用户态的逻辑调用树，并区分两个特权级中的协程节点。
...
4. **完成异步操作系统统一调试验证**。在 Async-os 上验证特权级切换管理与异步执行流还原的协同工作，实现跨特权级的断点组切换、协程追踪与逻辑调用树展示。
5. **扩展真实硬件与线上实验能力**。建立 VisionFive 2 真机调试链路，完成真实开发板的线上预约、分配和释放，为远程操作系统实验提供硬件基础。

### 6.2 未来展望

1. 实现调试配置的自动推导，减少地址范围、系统调用分发函数和内核出口函数等手动配置。
2. 优化特权级切换性能，使用 GDB finish 等机制减少逐指令单步带来的延迟。
3. 将 OpenOCD 启停、GDB 连接信息与串口日志纳入线上平台调度，实现预约驱动的远程硬件调试会话。
